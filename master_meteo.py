...
def ajouter_donnees():
    ...
    if _temperature and _humidity  and _capteur and request.method == 'POST':
        ...
        bindData_add_don = (_temperature, _humidity, _capteur)
        conn = mysql.connect()
...
def lire_donnees():
    try:
        ...
        cursor_read_don.execute("SELECT id, temperature, humidite, date, capture FROM donnees")
...
def modifier_donnees():
    try:
        ...
        if _id and _temperature and _humidity and _date and _capteur and request.method == 'PUT':
            ...
            bindData_mod_don = (_id, _temperature, _humidity, _date, _capteur)
            conn = mysql.connect()
...
def lire_utilisateurs():
    try:
        ...
        cursor_read_user.execute("SELECT id, nom_dutilisateur, nom, prenom, email, mot_de_passe FROM utilisateurs")
...
def modifier_utilisateur():
    try:
        ...
        if _id and _user and _nom and _prenom and _email and _password and request.method == 'PUT':
            ...
            bindData_mod_use = (_id, _user, _nom, _prenom, _email, _password)
            conn = mysql.connect()
